...
router.get('/item/:id', auth, async (req, res) => {
	try {
		const sql = 'SELECT * FROM information WHERE id = ? ORDER BY created_on DESC';
...
router.get('/today', auth, async (req, res) => {
	try {
		const sql = 'SELECT * FROM information WHERE created_on BETWEEN ? AND ? ORDER BY created_on DESC LIMIT 5';
