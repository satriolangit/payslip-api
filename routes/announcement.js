...
router.get('/item/:id', auth, async (req, res) => {
	try {
		const sql = 'SELECT * FROM announcement WHERE id = ? ORDER BY created_on DESC';
...
router.get('/today', auth, async (req, res) => {
	try {
		const sql = 'SELECT * FROM announcement WHERE created_on BETWEEN ? AND ? ORDER BY created_on DESC LIMIT 5';
...
router.get('/page/:page', auth, async (req, res) => {
	try {
		...
		if (page * numPerPage < totalRows) {
			...
			sql = 'SELECT * FROM announcement ORDER BY created_on';
...
router.post(
	...
	async (req, res) => {
		...
		try {
			...
			const sql =
				'INSERT INTO announcement (id, title, text, created_on, created_by) ' + 'VALUES (?, ?, ?, ?, ?)';
...
router.post(
	...
	async (req, res) => {
		...
		try {
			...
			const sql = 'UPDATE announcement SET title=?, text=?, updated_on=?, updated_by=? WHERE id=?';
			const now = moment(Date.now()).format('YYYY-MM-DD HH:mm:ss');
